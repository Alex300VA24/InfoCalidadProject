...
export default function Register({ roles }) {
    ...
    return <><Head title="Crear cuenta" /><AuthPanel icon="person_add" title="Cree su cuenta" description="Complete sus datos para solicitar acceso a la Plataforma de Calidad Académica."><form onSubmit={submit} className="space-y-5">
        ...
        <div className="grid gap-5 sm:grid-cols-2"><div><label htmlFor="password" className="mb-2 block text-sm font-bold text-ink-800">Contraseña</label><input id="password" type="password" value={data.password} onChange={(e) => setData('password', e.target.value)} autoComplete="new-password" required placeholder="Mín. 8 caracteres" className={fieldClass} />{errors.password && <p className={errorClass}>{errors.password}</p>}</div><div><label htmlFor="password_confirmation" className="mb-2 block text-sm font-bold text-ink-800">Confirmar contraseña</label><input id="password_confirmation" type="password" value={data.password_confirmation} onChange={(e) => setData('password_confirmation', e.target.value)} autoComplete="new-password" required placeholder="Repita la contraseña" className={fieldClass} />{errors.password_confirmation && <p className={errorClass}>{errors.password_confirmation}</p>}</div></div>
        <button type="submit" disabled={processing} className={`${primaryButtonClass} w-full`}>{processing ? 'Registrando…' : 'Crear cuenta'}</button>
