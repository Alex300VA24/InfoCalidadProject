...
export default function Login() {
    ...
    return <><Head title="Iniciar sesión" /><AuthPanel icon="login" title="Bienvenido" description="Ingrese sus credenciales institucionales para acceder a la Plataforma de Calidad Académica.">
        ...
        <form onSubmit={submit} className="space-y-5">
            ...
            <div><div className="mb-2 flex items-center justify-between"><label htmlFor="password" className="text-sm font-bold text-ink-800">Contraseña</label><Link href="/forgot-password" className={secondaryLinkClass}>¿Olvidó su contraseña?</Link></div><div className="relative"><input id="password" type={showPassword ? 'text' : 'password'} value={data.password} onChange={(e) => setData('password', e.target.value)} autoComplete="current-password" required placeholder="Ingrese su contraseña" className={`${fieldClass} pr-12`} /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'} aria-pressed={showPassword} className="absolute inset-y-0 right-0 px-4 text-ink-500 hover:text-brand-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400"><span className="material-symbols-outlined text-[20px]">{showPassword ? 'visibility_off' : 'visibility'}</span></button></div>{errors.password && <p className={errorClass}>{errors.password}</p>}</div>
            ...
            <button type="submit" disabled={processing} className={`${primaryButtonClass} w-full`}>{processing ? 'Ingresando…' : 'Ingresar al sistema'}</button>
            <p className="border-t border-ink-100 pt-5 text-center text-sm text-ink-600">¿No tiene una cuenta? <Link href="/register" className={secondaryLinkClass}>Registrarse</Link></p>
        </form>
        <Link href="/" className={`${secondaryLinkClass} mt-6 inline-flex items-center gap-2`}><span className="material-symbols-outlined text-[18px]">arrow_back</span>Volver a la página de inicio</Link>
    </AuthPanel></>
}
