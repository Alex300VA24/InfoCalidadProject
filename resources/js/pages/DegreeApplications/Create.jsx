...
export default function DegreeApplicationsCreate({ students, teachers, types }) {
            ...
            <div className="max-w-4xl mx-auto px-5 sm:px-8">
                <div className="flex justify-between items-end mb-6">
                    <div>
                        ...
                        <h2 className="text-3xl font-bold text-navy mt-2">Registrar Expediente</h2>
                        <p className="text-slate-500">Registra un expediente de grado o título profesional.</p>
                    </div>
                </div>
                        ...
                        <form onSubmit={submit}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Título de la Tesis / Trabajo</label>
                                    <input type="text" value={data.thesis_title} onChange={(e) => setData('thesis_title', e.target.value)} className="w-full rounded-lg border-slate-200 text-sm" />
                                    {errors.thesis_title && <p className="mt-1 text-sm text-red-600">{errors.thesis_title}</p>}
                                </div>
                                <div>
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea value={data.notes} onChange={(e) => setData('notes', e.target.value)} rows="3" className="w-full rounded-lg border-slate-200 text-sm" />
                                    {errors.notes && <p className="mt-1 text-sm text-red-600">{errors.notes}</p>}
                                </div>
                            </div>

                            <div className="flex justify-end">
                                <a href="/degrees/applications" className="px-4 py-2 border border-slate-300 text-slate-600 rounded-lg text-sm font-bold mr-2 hover:bg-slate-50 transition-colors">Cancelar</a>
                                <button type="submit" disabled={processing} className="px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-[#343d96] transition-colors">
                                    {processing ? 'Registrando...' : 'Registrar Expediente'}
                                </button>
                            </div>
                        </form>
                    </div>
...
DegreeApplicationsCreate.layout = (page) => <AppLayout>{page}</AppLayout>
