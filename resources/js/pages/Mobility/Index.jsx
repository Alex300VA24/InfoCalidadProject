...
export default function MobilityIndex({ applications, periods, types, statuses, filters }) {
    const query = (extra = {}) => {
    ...
    const applyFilter = (key, value) => {
        router.get('/mobility', query({ [key]: value }), {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        })
    ...
    const clearFilters = () => {
        router.get('/mobility', {}, { preserveState: true, preserveScroll: true, replace: true })
    }

    const hasFilters = Boolean(filters.academic_period_id || filters.type || filters.status)

    const statusColors = {
        aprobada: 'text-emerald-700 bg-emerald-100 border-emerald-200',
        finalizada: 'text-emerald-700 bg-emerald-100 border-emerald-200',
        en_curso: 'text-blue-700 bg-blue-100 border-blue-200',
        rechazada: 'text-red-700 bg-red-100 border-red-200',
        en_evaluacion: 'text-amber-700 bg-amber-100 border-amber-200',
    }
        ...
        <div className="page-enter">
            <div className="flex flex-wrap justify-between items-end gap-3 mb-6">
                <div>
                    ...
                    <h2 className="text-3xl font-bold text-navy mt-2">Solicitudes de Movilidad</h2>
                    <p className="text-slate-500">Gestión de movilidad nacional, internacional y becas.</p>
                </div>
                <div className="flex flex-wrap gap-2">
                    <a href="/mobility/agreements" className="inline-flex items-center gap-1.5 px-4 py-2 border border-slate-300 text-navy rounded-lg text-sm font-bold hover:bg-slate-50 transition-colors">
                        ...
                        <tbody className="text-sm divide-y divide-slate-100">
                            {applications.data.length > 0 ? applications.data.map((application) => (
                                <tr key={application.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-slate-500">{application.destination_institution ?? '—'}</td>
                                    <td className="px-6 py-4 text-slate-500">{application.academic_period?.name}</td>
                                    <td className="px-6 py-4 text-slate-500">{formatDate(application.application_date)}</td>
                                    <td className="px-6 py-4">
                                        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${statusColors[application.status] || 'text-slate-700 bg-slate-100 border-slate-200'}`}>
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-10 text-center text-slate-400">
                                        <p className="text-sm font-bold text-slate-600">No hay solicitudes registradas</p>
                                        <p className="text-xs mt-1">Registra la primera solicitud de movilidad</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
...
MobilityIndex.layout = (page) => <AppLayout>{page}</AppLayout>
