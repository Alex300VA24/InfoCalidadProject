...
export default function LoadsCreate({ periods, subjects, teachers, statuses, defaultPeriod }) {
                        ...
                        <form onSubmit={submit}>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={data.section} onChange={(e) => setData('section', e.target.value)} className="w-full rounded-lg border-slate-200 text-sm" />
                                ...
                                <div>
                                    ...
                                    <input type="number" value={data.hours} onChange={(e) => setData('hours', e.target.value)} min="0.5" max="100" step="0.5" required className="w-full rounded-lg border-slate-200 text-sm" />
                                    {errors.hours && <p className="mt-1 text-sm text-red-600">{errors.hours}</p>}
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1">Estado</label>
                                    <select value={data.status} onChange={(e) => setData('status', e.target.value)} required className="w-full rounded-lg border-slate-200 text-sm">
                            ...
                            <div className="flex justify-end">
                                <a href="/execution/loads" className="px-4 py-2 border border-slate-300 text-slate-600 rounded-lg text-sm font-bold mr-2 hover:bg-slate-50 transition-colors">Cancelar</a>
                                <button type="submit" disabled={processing} className="px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-[#343d96] transition-colors">
                                    {processing ? 'Registrando...' : 'Registrar Carga'}
                                </button>
                            </div>
                        </form>
                    </div>
...
LoadsCreate.layout = (page) => <AppLayout>{page}</AppLayout>
