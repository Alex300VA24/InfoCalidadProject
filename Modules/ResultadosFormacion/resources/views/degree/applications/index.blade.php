                        ...
                        <tbody class="text-sm divide-y divide-slate-100">
                            @forelse($applications as $application)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-6 py-4 font-bold text-navy">{{ $application->code }}</td>
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4 text-slate-500">{{ $application->resolution_number ?? '—' }}</td>
                                    <td class="px-6 py-4">
                                        <span class="px-3 py-1 rounded-full text-xs font-bold border
                                            {{ in_array($application->status, ['aprobado', 'otorgado']) ? 'text-emerald-700 bg-emerald-100 border-emerald-200' : ($application->status === 'observado' ? 'text-red-700 bg-red-100 border-red-200' : 'text-amber-700 bg-amber-100 border-amber-200') }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-10 text-center text-slate-400">
                                        <p class="text-sm font-bold text-slate-600">No hay expedientes registrados</p>
                                        <p class="text-xs mt-1">Registra el primer expediente de grado</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
