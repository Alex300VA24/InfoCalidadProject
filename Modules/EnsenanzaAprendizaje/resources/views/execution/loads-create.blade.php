    ...
    <x-slot name="header">
        <div class="flex justify-between items-end">
            <div>
                ...
                <h2 class="text-3xl font-bold text-navy mt-2">Registrar Carga Académica</h2>
                <p class="text-slate-500">Asigna una asignatura a un docente para el periodo académico.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div>
        <div class="max-w-4xl mx-auto px-5 sm:px-8">
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm">
                <div class="p-6">
                    <form method="POST" action="{{ route('execution.loads.store') }}">
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="section" value="{{ old('section') }}" placeholder="Ej. A" class="w-full rounded-lg border-slate-200 text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Horas</label>
                                <input type="number" name="hours" value="{{ old('hours', 4) }}" min="0.5" step="0.5" required class="w-full rounded-lg border-slate-200 text-sm">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-1">Estado</label>
                                <select name="status" class="w-full rounded-lg border-slate-200 text-sm">
                                    @foreach($statuses as $key => $label)
                                        <option value="{{ $key }}" {{ old('status', 'asignado') === $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end">
                            <a href="{{ route('execution.loads.index') }}" class="px-4 py-2 border border-slate-300 text-slate-600 rounded-lg text-sm font-bold mr-2 hover:bg-slate-50 transition-colors">Cancelar</a>
                            <button type="submit" class="px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-[#343d96] transition-colors">Registrar Carga</button>
                        </div>
                    </form>
                </div>
            </div>
