    ...
    <x-slot name="header">
        <div class="flex flex-wrap justify-between items-end gap-3">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('execution.loads.create') }}" class="inline-flex items-center gap-1.5 px-4 py-2 bg-accent text-ink font-black rounded shadow-md text-sm hover:brightness-95 transition-all">
                    ...
                    <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="flex items-center gap-2">
                            @if(count(request()->except(['page'])) > 0)
                                <a href="{{ route('execution.loads.index') }}" class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-semibold text-slate-500 hover:text-navy transition-colors" title="Quitar filtros">
