    ...
    <x-slot name="header">
        <div class="flex justify-between items-end">
            <div>
                ...
                <h2 class="text-3xl font-bold text-navy mt-2">Registrar Programa de Nivelación</h2>
                <p class="text-slate-500">Registra un programa de nivelación o recuperación para un estudiante.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div>
        <div class="max-w-4xl mx-auto px-5 sm:px-8">
            <div class="bg-white border border-slate-200 rounded-xl shadow-sm">
                <div class="p-6">
                    <form method="POST" action="{{ route('tutoring.remedial.store') }}">
                        @csrf
                            ...
                            <div>
                                ...
                                <select name="student_id" required class="w-full rounded-lg border-slate-200 text-sm">
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student->id }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>{{ $student->codigo }} - {{ $student->user?->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                        ...
                        <div class="flex justify-end">
                            <a href="{{ route('tutoring.remedial.index') }}" class="px-4 py-2 border border-slate-300 text-slate-600 rounded-lg text-sm font-bold mr-2 hover:bg-slate-50 transition-colors">Cancelar</a>
                            <button type="submit" class="px-4 py-2 bg-navy text-white rounded-lg text-sm font-semibold hover:bg-[#343d96] transition-colors">Registrar Programa</button>
                        </div>
                    </form>
                </div>
            </div>
